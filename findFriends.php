...
$query = "SELECT users.id, username, firstname, lastname FROM users RIGHT JOIN friends ON users.id = friend_id WHERE sender_id != '".$userid."' UNION SELECT users.id, username, firstname, lastname FROM users RIGHT JOIN friends ON users.id = sender_id WHERE friend_id != '".$userid."'";
$result = $conn->query($query);
while($row = $result->fetch_assoc()){
	array_push($all_users, $row);
}
